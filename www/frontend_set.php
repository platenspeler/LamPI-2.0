...
function fill_weatherdb($weatherdb)
{
	...
	if (!$mysqli->query("DROP TABLE IF EXISTS weatherdb") ||
    	!$mysqli->query("CREATE TABLE weatherdb(id INT, timestamp CHAR(20), name CHAR(20), location CHAR(20), brand CHAR(20), address CHAR(20), channel CHAR(8), temperature CHAR(8), humidity CHAR(8), airpressure CHAR(8), windspeed CHAR(8), winddirection CHAR(8), rain INT )") )
	{
	...
	{	
		if (!$mysqli->query("INSERT INTO weatherdb (id, timestamp, location, brand, address, channel, temperature, humidity, airpressure, windspeed, winddirection, rain ) VALUES ('" 
					. $weatherdb[$i][id]. "','" 
...
function print_weatherdb($weatherdb)
{
	...
	for ($i=0; $i < count($weatherdb); $i++) 
	{
		echo("index: $i id: ".$weatherdb[$i]['id']
				...
				.", airpressure: ".$weatherdb[$i][airpressure]
